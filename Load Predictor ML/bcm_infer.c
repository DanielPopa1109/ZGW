...
void BCM_Infer(const float in[BCM_INPUT_SIZE],float raw[BCM_OUTPUT_SIZE]){
    float h1[BCM_H1_SIZE],h2[BCM_H2_SIZE],h3[BCM_H3_SIZE];uint16_t n;
    dense(in,bcm_net_0_weight,bcm_net_0_bias,BCM_INPUT_SIZE,BCM_H1_SIZE,h1);for(n=0;n<BCM_H1_SIZE;n++)h1[n]=relu(h1[n]);
    dense(h1,bcm_net_2_weight,bcm_net_2_bias,BCM_H1_SIZE,BCM_H2_SIZE,h2);for(n=0;n<BCM_H2_SIZE;n++)h2[n]=relu(h2[n]);
    dense(h2,bcm_net_4_weight,bcm_net_4_bias,BCM_H2_SIZE,BCM_H3_SIZE,h3);for(n=0;n<BCM_H3_SIZE;n++)h3[n]=relu(h3[n]);
