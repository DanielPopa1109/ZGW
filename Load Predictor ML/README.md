...
The normal class includes dedicated hard-negative samples based on LoadSim's
healthy Driving profile. They cover coordinated load modulation, accessory
steps, integer CAN-signal quantization, and currents up to the healthy 88% of
rating envelope. These samples remain labelled `normal` with
`fault_soon = 0`; actual precursor trajectories remain positive samples.

Synthetic dataset changes have their own schema version. `train.py` rejects a
checkpoint from an older dataset version and starts with fresh model and
optimizer state, preventing a previously converged near-zero learning rate from
blocking adaptation to the new distribution.
