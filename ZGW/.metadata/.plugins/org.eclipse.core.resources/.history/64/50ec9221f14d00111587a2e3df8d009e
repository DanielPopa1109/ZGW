...
#define GATEWAYSWC_UNUSED(x)                          ((void)(x))
...
#define GATEWAYSWC_CAN_RX_FIRST                       COM_SIG_RX_CENTRALLOCKDATA_VIBRATIONSENSORSTATUS
#define GATEWAYSWC_CAN_RX_LAST                        COM_SIG_RX_BATTCAPRES_CAPACITYAH

#define GATEWAYSWC_CANFD_RX_FIRST                     COM_SIG_RX_CANFD_PDM4_LOADSTATUS_PDM4_LOADSTATUS_01
#define GATEWAYSWC_CANFD_RX_LAST                      COM_SIG_RX_CANFD_PDM4_DIAGRESPONSE_PDM4_DIAGRESP_BYTE07
...
static uint16 GatewaySwc_EthTimerMs;
...
{
    { GATEWAYSWC_CAN_RX_FIRST,   GATEWAYSWC_CAN_RX_LAST,   GATEWAYSWC_BUS_CAN },
...
void GatewaySwc_MainFunction(void)
{
    ...
    GatewaySwc_EthTimerMs += GATEWAYSWC_MAIN_PERIOD_MS;
    ...
    if (GatewaySwc_EthTimerMs >= GATEWAYSWC_ETH_PERIOD_MS)
    {
        GatewaySwc_EthTimerMs = 0u;
...
static void GatewaySwc_LoadDefaultCommands(void)
{
    ...
    GatewaySwc_Command.vehicleStatus = 1u;
    GatewaySwc_Command.nmPn1 = 1u;
}
...
static void GatewaySwc_GenerateCanOutputs(void)
{
    ...
    crc = ((GatewaySwc_Command.pdmCommandLoad[0u][0u] ^
            GatewaySwc_Command.pdmCommandLoad[0u][1u] ^
            GatewaySwc_Command.pdmCommandLoad[0u][2u] ^
            alive) & 0xFFu);
    ...
    (void)GatewaySwc_SendU32(COM_SIG_TX_LOADREQUEST_LOADREQUESTSTATUS, GatewaySwc_Command.pdmCommandLoad[0u][0u]);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LOADREQUEST_LOADREQUESTDATAID, GatewaySwc_Command.pdmCommandLoad[0u][1u]);
                                    ...
                                    Com_SignalIdType lastSignalId)
{
    ...
    for (signalId = firstSignalId; signalId <= lastSignalId; signalId++)
    {
        ...
        if (GatewaySwc_ReceiveU32(signalId, &value) != E_OK)
        {
            continue;
        }
...
static void GatewaySwc_FlushEthFrame(uint16 *len)
{
    ...
    if (SoAd_IfTransmit((SoAd_SoConIdType)GATEWAYSWC_ETH_SOCON_ID,
                        NULL_PTR,
