...
void lwip_geth_private_Phy_Dp83825i_mainFunction_100ms(void)
{
    ...
    switch (Dp83825i_Status.state)
    {
        ...
        {
            if ((lwip_geth_private_Phy_Dp83825i_write_mdio_reg(DP83825I_PHY_ADDR, DP83825I_REG_ANAR, 0x01E1u) == 0u) ||
                (lwip_geth_private_Phy_Dp83825i_write_mdio_reg(
                    DP83825I_PHY_ADDR,
                    DP83825I_REG_BMCR,
                    DP83825I_BMCR_AUTONEG_ENABLE | DP83825I_BMCR_RESTART_AUTONEG) == 0u))
            {
        ...
        case LWIP_GETH_PHY_DP83825I_STATE_WAIT_AUTONEG:
        {
            ...
            if (((bmsrSecond & DP83825I_BMSR_LINK_STATUS) != 0u) ||
                ((physts & DP83825I_PHYSTS_LINK) != 0u))
            {
                Dp83825i_Status.linkUp = 1u;
                Dp83825i_Status.speed100 = ((physts & DP83825I_PHYSTS_SPEED_10) == 0u) ? 1u : 0u;
                Dp83825i_Status.fullDuplex = ((physts & DP83825I_PHYSTS_FULL_DUPLEX) != 0u) ? 1u : 0u;
                Dp83825i_Status.autonegDone = ((bmsrSecond & DP83825I_BMSR_AUTONEG_DONE) != 0u) ? 1u : 0u;
                Dp83825i_Status.linkUpCnt++;
                Dp83825i_Status.state = LWIP_GETH_PHY_DP83825I_STATE_LINK_UP;
            }
            else
...
uint32 lwip_geth_private_Phy_Dp83825i_link_status(void)
{
    ...
    if (Dp83825i_Status.fullDuplex != 0u)
    {
        link_status.B.LNKMOD = 1u;
    }

    if (Dp83825i_Status.speed100 != 0u)
    {
        link_status.B.LNKSPEED = 1u;
    }
