...
static Std_ReturnType Fls_ProgramOnePage(Fls_AddressType address, const uint8 *data)
{
...
static Std_ReturnType Fls_EraseOneSector(Fls_AddressType address)
{
    uint32 physicalAddress = Fls_GetPhysicalAddress(address);
    uint16 password;
#if (FLS_DISABLE_INTERRUPTS_FOR_COMMAND == STD_ON)
...
void Fls_MainFunction(void)
{
    ...
    switch (Fls_State.job)
    {
        case FLS_JOB_READ:
            ...
            memcpy(dst, (const void *)Fls_GetPhysicalAddress(current), (size_t)(Fls_State.length - Fls_State.progress));
            Fls_FinishJob(MEMIF_JOB_OK);
            break;

        case FLS_JOB_WRITE:
            src = &Fls_State.src[Fls_State.progress];
            result = Fls_ProgramOnePage(current, src);
            if (result != E_OK)
            {
        ...
        case FLS_JOB_ERASE:
            result = Fls_EraseOneSector(current);
            if (result != E_OK)
            {
                MemStack_ReportError(MEMSTACK_MODULE_ID_FLS, FLS_API_MAIN, FLS_E_VERIFY, current);
                Fls_FinishJob(MEMIF_JOB_FAILED);
