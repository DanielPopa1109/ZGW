...
#define GATEWAYSWC_FRAME_CPU_PERF                     0x06u
#define GATEWAYSWC_FRAME_VEHICLESTATE                 0x07u

#define GATEWAYSWC_AI_MODEL_GLOBAL_LEN                61u
#define GATEWAYSWC_AI_MODEL_CHANNEL_LEN               43u
#define GATEWAYSWC_CPU_PERF_HEADER_LEN                8u
...
#define GATEWAYSWC_CROSS_CORE_LOCK_TIMEOUT            100000u
#define GATEWAYSWC_ETH_TX_QUEUE_SIZE                  32u
#define GATEWAYSWC_ETH_RX_QUEUE_SIZE                  8u
#define GATEWAYSWC_ETH_TX_RESULT_QUEUE_SIZE           32u
#define GATEWAYSWC_ETH_RX_QUEUE_MAX_PAYLOAD           GATEWAYSWC_ETH_MAX_PAYLOAD
#define GATEWAYSWC_ETH_TX_DRAIN_BUDGET                4u
...
#define GATEWAYSWC_ETH_TX_KIND_CPU_PERF               5u
#define GATEWAYSWC_ETH_TX_KIND_VEHICLESTATE           6u

#define GATEWAYSWC_DIAG_PERIOD_MS                     1000u
#define GATEWAYSWC_ETH_RX_TIMEOUT_MS                  3000u
#define GATEWAYSWC_ETH_PERIOD_NS                      ((uint64)GATEWAYSWC_ETH_PERIOD_MS * TIMEBASE_NS_PER_MS)
#define GATEWAYSWC_AI_MODEL_PUBLISH_PERIOD_MS         200u
#define GATEWAYSWC_CPU_PERF_PUBLISH_PERIOD_MS         1000u
...
#define GATEWAYSWC_ROUTE_PHASE_IDLE                   0u
#define GATEWAYSWC_ROUTE_PHASE_RECEIVE                1u
#define GATEWAYSWC_ROUTE_PHASE_RECEIVED               2u
#define GATEWAYSWC_ROUTE_PHASE_SEND                   3u
...
typedef struct
{
        ...
        uint16 len;
        uint8 data[GATEWAYSWC_ETH_MAX_PAYLOAD];
} GatewaySwc_EthTxJobType;
...
typedef struct
{
        ...
        TcpIp_SockAddrType remoteAddr;
        uint16 len;
        uint8 data[GATEWAYSWC_ETH_RX_QUEUE_MAX_PAYLOAD];
} GatewaySwc_EthRxJobType;
...
static volatile uint8 GatewaySwc_Initialized = 0u;
...
static GatewaySwc_EthTxJobType GatewaySwc_EthTxQueue[GATEWAYSWC_ETH_TX_QUEUE_SIZE];
static GatewaySwc_EthTxResultType GatewaySwc_EthTxResultQueue[GATEWAYSWC_ETH_TX_RESULT_QUEUE_SIZE];
static GatewaySwc_EthRxJobType GatewaySwc_EthRxQueue[GATEWAYSWC_ETH_RX_QUEUE_SIZE];
static volatile uint8 GatewaySwc_EthTxHead;
static volatile uint8 GatewaySwc_EthTxTail;
static volatile uint8 GatewaySwc_EthTxResultHead;
static volatile uint8 GatewaySwc_EthTxResultTail;
static volatile uint8 GatewaySwc_EthRxHead;
static volatile uint8 GatewaySwc_EthRxTail;
static IfxCpu_spinLock GatewaySwc_CrossCoreLock;
static boolean GatewaySwc_CrossCoreIrqState[GATEWAYSWC_CPU_CORE_COUNT];
static volatile uint8 GatewaySwc_CrossCoreLockOwned[GATEWAYSWC_CPU_CORE_COUNT];

static uint16 GatewaySwc_RouteTimerMs;
static uint16 GatewaySwc_OutputTimerMs;
static uint16 GatewaySwc_AiModelPublishTimerMs;
static uint16 GatewaySwc_CpuPerfPublishTimerMs;
...
static volatile uint8 GatewaySwc_McuStatusTxPending = 0u;
...
static uint32 GatewaySwc_EthRxAgeMs[GATEWAYSWC_ETH_RX_MESSAGE_COUNT];
...
volatile uint16 GatewaySwc_DebugRouteIndex = 0xFFFFu;
volatile uint16 GatewaySwc_DebugRouteCount = 0u;
volatile uint16 GatewaySwc_DebugRouteRxSignalId = 0u;
volatile uint16 GatewaySwc_DebugRouteTxSignalId = 0u;
volatile uint8 GatewaySwc_DebugRoutePhase = GATEWAYSWC_ROUTE_PHASE_IDLE;
volatile uint8 GatewaySwc_DebugRouteReceiveResult = E_NOT_OK;
volatile uint8 GatewaySwc_DebugRouteSendResult = E_NOT_OK;
volatile uint32 GatewaySwc_DebugRouteValue = 0u;
...
volatile uint32 GatewaySwc_DebugEthTxQueueDropped = 0u;
volatile uint32 GatewaySwc_DebugEthTxResultQueueDropped = 0u;
volatile uint32 GatewaySwc_DebugEthRxQueueDropped = 0u;
volatile uint32 GatewaySwc_DebugCrossCoreLockTimeout = 0u;
...
#define GATEWAYSWC_DEBUG_ASSIGN(lhs, rhs) do { (lhs) = (rhs); } while (0)
#define GATEWAYSWC_DEBUG_INC(lhs) do { (lhs)++; } while (0)
#else
#define GATEWAYSWC_DEBUG_ASSIGN(lhs, rhs) do { } while (0)
#define GATEWAYSWC_DEBUG_INC(lhs) do { } while (0)
#endif
...
Dcm_ReturnType GatewaySwc_SetDiagnosticCommunicationControl(uint8 controlType, uint8 communicationType)
{
    ...
    if (((controlType == 0x00u) || (controlType == 0x02u)) &&
            (GatewaySwc_NormalCommunicationTxEnabled == FALSE))
    {
        Com_TriggerFullComRestartBurst(COMM_CH_CAN);
        Com_TriggerFullComRestartBurst(COMM_CH_CANFD);
        Com_TriggerFullComRestartBurst(COMM_CH_LIN);
    }
...
SoAd_ReturnType GatewaySwc_RequestSoAdIfTransmit(SoAd_SoConIdType soConId,
        ...
        uint16 len)
{
    EthStartupTiming_Capture(ETHSTARTUPTIMING_EVENT_FIRST_APP_TX_REQUEST);
...
{
        { COM_RX_PDU_CENTRALLOCKDATA,        COM_RX_PDU_DMUSTATUS,                       GATEWAYSWC_BUS_CAN },
        { COM_RX_PDU_BATTFULLSTAT,           COM_RX_PDU_BATTFULLSTAT,                    GATEWAYSWC_BUS_CAN },
        ...
        { GATEWAYSWC_ETH_RX_PDU_COMMAND_SIGNAL, GATEWAYSWC_ETH_RX_PDU_COMMAND_BLOCK,       GATEWAYSWC_BUS_ETH }
};
...
static void GatewaySwc_SendLinSdat(const GatewaySwc_SdatTimeType *time);
...
static void GatewaySwc_PublishEthernetVehicleState(void);
static void GatewaySwc_PublishAiModelResult(void);
static void GatewaySwc_PublishCpuPerf(void);
...
static void GatewaySwc_McuStatusStorePacketForWriteAll(const uint8 *packet);
...
static void GatewaySwc_ProcessEthTxQueueCore2(void);
static void GatewaySwc_HandleEthRxMessage(uint8 soConId,
        const TcpIp_SockAddrType *remoteAddr,
        const uint8 *data,
        uint16 len);
static uint8 GatewaySwc_QueueEthTx(uint8 kind,
        SoAd_SoConIdType soConId,
        const TcpIp_SockAddrType *remoteAddr,
        const uint8 *data,
        uint16 len);
static boolean GatewaySwc_GetEthTxCodingPduId(uint8 kind, PduIdType *pduId);
static boolean GatewaySwc_IsEthTxKindEnabled(uint8 kind);
static uint8 GatewaySwc_PopEthTx(GatewaySwc_EthTxJobType *job);
static uint8 GatewaySwc_QueueEthTxResult(uint8 kind,
        uint8 openResult,
        uint16 len,
        sint32 result);
static uint8 GatewaySwc_PopEthTxResult(GatewaySwc_EthTxResultType *result);
static uint8 GatewaySwc_QueueEthRx(uint8 soConId,
        const TcpIp_SockAddrType *remoteAddr,
        const uint8 *data,
        uint16 len);
static uint8 GatewaySwc_PopEthRx(GatewaySwc_EthRxJobType *job);
static void GatewaySwc_ApplyEthTxResult(const GatewaySwc_EthTxResultType *result);
static SoAd_ReturnType GatewaySwc_Core2TransmitSoAd(const GatewaySwc_EthTxJobType *job,
        uint8 *openResult);
static sint32 GatewaySwc_Core2TransmitMcuStatus(const GatewaySwc_EthTxJobType *job);
static uint8 GatewaySwc_EnterCrossCoreCritical(void);
static void GatewaySwc_ExitCrossCoreCritical(void);
...
#else
#define GatewaySwc_UpdateDtcDebugQueueState() ((void)0)
#endif
...
static void GatewaySwc_CaptureEthRxMessage(uint8 soConId,
        ...
        uint16 len);
#else
#define GatewaySwc_CaptureEthRxMessage(soConId, remoteAddr, data, len) ((void)0)
#endif
...
void GatewaySwc_Init(void)
{
    ...
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteIndex, 0xFFFFu);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteCount, GatewaySwc_Status.configuredRoutes);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteRxSignalId, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteTxSignalId, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRoutePhase, GATEWAYSWC_ROUTE_PHASE_IDLE);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteReceiveResult, E_NOT_OK);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteSendResult, E_NOT_OK);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteValue, 0u);
    ...
    GatewaySwc_AiModelPublishTimerMs = 0u;
    GatewaySwc_CpuPerfPublishTimerMs = 0u;
    GatewaySwc_NextEthPublishTimeNs = 0ull;
    GatewaySwc_McuStatusInit();
    GatewaySwc_EthTxHead = 0u;
    GatewaySwc_EthTxTail = 0u;
    GatewaySwc_EthTxResultHead = 0u;
    GatewaySwc_EthTxResultTail = 0u;
    GatewaySwc_EthRxHead = 0u;
    GatewaySwc_EthRxTail = 0u;
    GatewaySwc_McuStatusTxPending = 0u;
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugEthTxQueueDropped, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugEthTxResultQueueDropped, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugEthRxQueueDropped, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugCrossCoreLockTimeout, 0u);
    GatewaySwc_DiagTimerMs = 0u;
    for (ethRxIndex = 0u; ethRxIndex < GATEWAYSWC_ETH_RX_MESSAGE_COUNT; ethRxIndex++)
    {
        GatewaySwc_EthRxAgeMs[ethRxIndex] = 0u;
    }
    GatewaySwc_LoadRequestAliveCounter = 0u;
    GatewaySwc_DtcTransitionHead = 0u;
    GatewaySwc_DtcTransitionTail = 0u;
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcDropped, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcFlushed, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastDtc, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastStatus, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastOpenResult, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastTxResult, SOAD_NOT_OK);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcQueueLockTimeout, 0u);
    ...
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureEnable, GATEWAYSWC_ETH_RX_CAPTURE_DEFAULT_ENABLE);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureFilterMode, GATEWAYSWC_ETH_RX_CAPTURE_FILTER_OFF);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureFilterSoConId, GATEWAYSWC_ETH_SOCON_ID);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureMaxLen, GATEWAYSWC_ETH_RX_CAPTURE_MAX_PAYLOAD);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureWriteIndex, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureLastIndex, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureCounter, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureDroppedCounter, 0u);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureTruncatedCounter, 0u);
...
void GatewaySwc_MainFunction(void)
{
    ...
    CpuPerf_Start(CPUPERF_ID_GATEWAY_MAIN_C0, &cpuPerfCtx);

    GatewaySwc_ProcessCore0Async();
    ...
    if (GatewaySwc_NormalCommunicationTxEnabled != FALSE)
    {
        GatewaySwc_AiModelPublishTimerMs += GATEWAYSWC_MAIN_PERIOD_MS;
        GatewaySwc_CpuPerfPublishTimerMs += GATEWAYSWC_MAIN_PERIOD_MS;

        if (GatewaySwc_AiModelPublishTimerMs >= GATEWAYSWC_AI_MODEL_PUBLISH_PERIOD_MS)
        {
            GatewaySwc_AiModelPublishTimerMs = 0u;
            GatewaySwc_PublishAiModelResult();
        }

        if (GatewaySwc_CpuPerfPublishTimerMs >= GATEWAYSWC_CPU_PERF_PUBLISH_PERIOD_MS)
        {
            GatewaySwc_CpuPerfPublishTimerMs = 0u;
            GatewaySwc_PublishCpuPerf();
        }
    }
    ...
    CpuPerf_Stop(CPUPERF_ID_GATEWAY_MAIN_C0, &cpuPerfCtx);
}
...
void GatewaySwc_EthernetMainFunction(void)
{
    ...
    CpuPerf_Start(CPUPERF_ID_GATEWAY_ETH_MAIN_C2, &cpuPerfCtx);
    GatewaySwc_ProcessEthTxQueueCore2();
    CpuPerf_Stop(CPUPERF_ID_GATEWAY_ETH_MAIN_C2, &cpuPerfCtx);
}
...
void GatewaySwc_EthRxIndication(uint8 soConId,
        ...
        uint16 len)
{
    EthStartupTiming_CaptureFirstRx(ETHSTARTUPTIMING_RX_IPV4_UDP);
    (void)GatewaySwc_QueueEthRx(soConId, remoteAddr, data, len);
}
        ...
        uint16 len)
{
    ...
    else if ((data[3] == GATEWAYSWC_FRAME_COMMAND_BLOCK) && (len >= 32u))
    {
        uint16 idx = 4u;
        uint8 p;
        uint8 l;

        GatewaySwc_EthRxAgeMs[1u] = 0u;
        ...
        uint16 len)
{
    ...
    if (data == NULL_PTR)
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_EthRxCaptureDroppedCounter);
        return;
    }
    ...
    if (copyLen > configuredMaxLen)
    {
        ...
        GATEWAYSWC_DEBUG_INC(GatewaySwc_EthRxCaptureTruncatedCounter);
    }
    ...
    {
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureData[slot][i], data[i]);
    }

    for (i = copyLen; i < GATEWAYSWC_ETH_RX_CAPTURE_MAX_PAYLOAD; i++)
    {
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureData[slot][i], 0u);
    }

    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureSoConId[slot], soConId);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureLen[slot], len);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureCopiedLen[slot], copyLen);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureMainCycle[slot], GatewaySwc_Status.mainCycles);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureRemoteAddrPtr[slot], (uint32)remoteAddr);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureLastIndex, slot);
    GATEWAYSWC_DEBUG_INC(GatewaySwc_EthRxCaptureCounter);
    ...
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_EthRxCaptureWriteIndex, slot);
}
#endif
...
static void GatewaySwc_RouteSignals(void)
{
    ...
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteCount, (uint16)(sizeof(GatewaySwc_RouteMap) / sizeof(GatewaySwc_RouteMap[0])));
    for (i = 0u; i < (uint16)(sizeof(GatewaySwc_RouteMap) / sizeof(GatewaySwc_RouteMap[0])); i++)
    {
        rxSignalId = GatewaySwc_RouteMap[i].rxSignalId;
        txSignalId = GatewaySwc_RouteMap[i].txSignalId;

        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteIndex, i);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteRxSignalId, rxSignalId);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteTxSignalId, txSignalId);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteReceiveResult, E_NOT_OK);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteSendResult, E_NOT_OK);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRoutePhase, GATEWAYSWC_ROUTE_PHASE_RECEIVE);

        receiveResult = GatewaySwc_ReceiveU32(rxSignalId, &value);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteReceiveResult, (uint8)receiveResult);
        if (receiveResult == E_OK)
        {
            GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteValue, value);
            GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRoutePhase, GATEWAYSWC_ROUTE_PHASE_SEND);

            sendResult = GatewaySwc_SendU32(txSignalId, value);
            GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRouteSendResult, (uint8)sendResult);
            GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRoutePhase, GATEWAYSWC_ROUTE_PHASE_SENT);
        ...
        {
            GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugRoutePhase, GATEWAYSWC_ROUTE_PHASE_RECEIVED);
        }
    }
...
static void GatewaySwc_GenerateCanOutputs(void)
{
    ...
    counter++;
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugLoadRequestCounter, counter);
    ...
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugLoadRequestChanged, hasChanged);
    ...
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugLoadRequestAlive, alive);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugLoadRequestCrc, crc);
...
static void GatewaySwc_PublishEthernetSummary(void)
{
    ...
    GatewaySwc_PublishEthernetVehicleState();
}
...
static void GatewaySwc_PublishEthernetVehicleState(void)
{
    ...
    payload[0u] = GATEWAYSWC_MAGIC0;
    payload[1u] = GATEWAYSWC_MAGIC1;
    payload[2u] = GATEWAYSWC_MAGIC2;
    payload[3u] = GATEWAYSWC_FRAME_VEHICLESTATE;
    GatewaySwc_StoreU32(payload, 4u, GatewaySwc_Status.mainCycles);
    GatewaySwc_StoreU32(payload, 8u, GatewaySwc_Command.vehicleStatus);

    GatewaySwc_Status.ethLastPayloadLength = (uint16)sizeof(payload);
    if (GatewaySwc_QueueEthTx(GATEWAYSWC_ETH_TX_KIND_VEHICLESTATE,
            (SoAd_SoConIdType)GATEWAYSWC_ETH_SOCON_ID,
            NULL_PTR,
            payload,
            (uint16)sizeof(payload)) == FALSE)
    {
        GatewaySwc_Status.ethLastOpenResult = 0u;
...
static void GatewaySwc_PublishCpuPerf(void)
{
    ...
    maxEntries = (uint8)((GATEWAYSWC_ETH_MAX_PAYLOAD - GATEWAYSWC_CPU_PERF_HEADER_LEN -
            CPUPERF_RESPONSE_HEADER_LEN) / CPUPERF_RESPONSE_ENTRY_LEN);
    if (maxEntries == 0u)
    {
        return;
    }
    if (maxEntries > CPUPERF_RESPONSE_MAX_ENTRIES)
    {
        maxEntries = CPUPERF_RESPONSE_MAX_ENTRIES;
    }

    for (startId = 0u; startId < (uint8)CPUPERF_ID_COUNT; startId = (uint8)(startId + maxEntries))
    {
        GatewaySwc_EthBuffer[0u] = GATEWAYSWC_MAGIC0;
        GatewaySwc_EthBuffer[1u] = GATEWAYSWC_MAGIC1;
        GatewaySwc_EthBuffer[2u] = GATEWAYSWC_MAGIC2;
        GatewaySwc_EthBuffer[3u] = GATEWAYSWC_FRAME_CPU_PERF;
        GatewaySwc_StoreU32(GatewaySwc_EthBuffer, 4u, GatewaySwc_Status.mainCycles);

        if (CpuPerf_BuildSnapshotPayload(
                startId,
                maxEntries,
                &GatewaySwc_EthBuffer[GATEWAYSWC_CPU_PERF_HEADER_LEN],
                &payloadLen) != E_OK)
        {
            continue;
        }

        payloadLen = (uint16)(payloadLen + GATEWAYSWC_CPU_PERF_HEADER_LEN);
        GatewaySwc_Status.ethLastPayloadLength = payloadLen;
        if (GatewaySwc_QueueEthTx(GATEWAYSWC_ETH_TX_KIND_CPU_PERF,
                (SoAd_SoConIdType)GATEWAYSWC_ETH_SOCON_ID,
                NULL_PTR,
...
static void GatewaySwc_McuStatusStorePacketForWriteAll(const uint8 *packet)
{
    ...
    if (memcmp(NvM_McuStatus_Ram, packet, GATEWAYSWC_MCU_STATUS_NVM_IMAGE_SIZE) != 0)
    {
        memcpy(NvM_McuStatus_Ram, packet, GATEWAYSWC_MCU_STATUS_NVM_IMAGE_SIZE);
        (void)NvM_SetRamBlockStatus(NVM_BLOCK_ID_MCU_STATUS, TRUE);
    }
}
...
static void GatewaySwc_McuStatusSendPacket(void)
{
    ...
    if (GatewaySwc_IsEthTxKindEnabled(GATEWAYSWC_ETH_TX_KIND_MCU_STATUS) == FALSE)
    {
        return;
    }
    ...
    GatewaySwc_McuStatusStorePacketForWriteAll(packet);
    GatewaySwc_Status.ethLastPayloadLength = GATEWAYSWC_MCU_STATUS_PACKET_LENGTH;
    if (GatewaySwc_QueueEthTx(GATEWAYSWC_ETH_TX_KIND_MCU_STATUS,
            0u,
            &remoteAddr,
            packet,
            GATEWAYSWC_MCU_STATUS_PACKET_LENGTH) != FALSE)
    {
        GatewaySwc_McuStatusTxPending = 1u;
    }
    else
    {
...
Std_ReturnType GatewaySwc_ReadDid(uint16 did, uint8 *data, Dcm_PduLengthType *dataLen)
{
    ...
    if (did != DCM_DID_MCU_DATA_PACKET)
    {
        return E_NOT_OK;
    }

    if (*dataLen < GATEWAYSWC_MCU_STATUS_NVM_IMAGE_SIZE)
    {
        return E_NOT_OK;
    }

    memcpy(data, NvM_McuStatus_Ram, GATEWAYSWC_MCU_STATUS_NVM_IMAGE_SIZE);
    *dataLen = GATEWAYSWC_MCU_STATUS_NVM_IMAGE_SIZE;
        ...
        Com_SignalIdType lastSignalId)
{
    ...
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishBus, (uint8)bus);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishFirstSignalId, firstSignalId);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishLastSignalId, lastSignalId);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishInvalidRange, 0u);

    if (firstSignalId > lastSignalId)
    {
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishInvalidRange, 1u);
        return;
    }

    for (signalId = firstSignalId; signalId <= lastSignalId; signalId++)
    {
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishSignalId, signalId);
        receiveResult = GatewaySwc_ReceiveU32(signalId, &value);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishReceiveResult, (uint8)receiveResult);

        if (receiveResult != E_OK)
        {
            continue;
        }
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishValue, value);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugPublishLen, len);
...
static void GatewaySwc_SendLinSdat(const GatewaySwc_SdatTimeType *time)
{
    ...
    /* The LIN frame preserves the canonical SDAT layout and scaling:
     * year is relative to 2000 and milliseconds use 4 ms per bit. */
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_MINUTE, time->minute);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_TIMESOURCE, time->timeSource);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_SECOND, time->second);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_UTCVALID, time->utcValid);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_DEFAULTTIME, time->defaultTime);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_YEAR, time->year);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_NVMRESTORED, time->nvmRestored);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_DAY, time->day);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_HOUR, time->hour);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_MONTH, time->month);
    (void)GatewaySwc_SendU32(COM_SIG_TX_LIN_ZGW_SDAT_MILLISECOND, time->millisecond);
}
...
static void GatewaySwc_ResetRxDiagnostics(void)
{
    ...
    for (i = 0u; i < (uint16)GATEWAYSWC_RX_MESSAGE_DIAG_COUNT; i++)
    {
        ...
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxMessageDiagDebugPduId[i], 0u);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxMessageDiagDebugStatus[i], GATEWAYSWC_RX_DIAG_STATUS_OK);
    ...
    for (i = 0u; i < (uint16)GATEWAYSWC_RX_SIGNAL_DIAG_COUNT; i++)
    {
        ...
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxSignalDiagDebugSignalId[i], 0u);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxSignalDiagDebugStatus[i], GATEWAYSWC_RX_DIAG_STATUS_OK);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxSignalDiagDebugValue[i], 0u);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxSignalDiagDebugInvalidValue[i], 0u);
...
static uint8 GatewaySwc_IsNmActiveForBus(GatewaySwc_BusType bus)
{
    ...
    switch (bus)
    {
        ...
        case GATEWAYSWC_BUS_ETH:
            return GatewaySwc_IsEthComActive();
...
static void GatewaySwc_UpdateRxDiagnostics(void)
{
    ...
    for (i = 0u; i < (uint16)GATEWAYSWC_RX_MESSAGE_DIAG_COUNT; i++)
    {
        ...
        {
            if (bus == GATEWAYSWC_BUS_ETH)
            {
                ethRxIndex = (uint16)((uint16)pduId -
                        (uint16)GATEWAYSWC_ETH_RX_PDU_COMMAND_SIGNAL);
                if (ethRxIndex < GATEWAYSWC_ETH_RX_MESSAGE_COUNT)
                {
                    if ((CodingApp_IsRxMessageExpected(i) != FALSE) &&
                            (GatewaySwc_IsEthComActive() != FALSE) &&
                            (GatewaySwc_IsRxDiagSuppressedForBus(bus) == FALSE))
                    {
                        if (GatewaySwc_EthRxAgeMs[ethRxIndex] <=
                                (0xFFFFFFFFu - GATEWAYSWC_DIAG_PERIOD_MS))
                        {
                            GatewaySwc_EthRxAgeMs[ethRxIndex] += GATEWAYSWC_DIAG_PERIOD_MS;
                        }

                        if (GatewaySwc_EthRxAgeMs[ethRxIndex] >= GATEWAYSWC_ETH_RX_TIMEOUT_MS)
                        {
            ...
            else if (Com_GetRxPduDiagStatus(pduId, &comStatus) == E_OK)
            {
        ...
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxMessageDiagDebugPduId[i], (uint16)pduId);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxMessageDiagDebugStatus[i], status);
    ...
    for (i = 0u; i < (uint16)GATEWAYSWC_RX_SIGNAL_DIAG_COUNT; i++)
    {
        ...
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxSignalDiagDebugSignalId[i], (uint16)signalId);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxSignalDiagDebugStatus[i], status);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxSignalDiagDebugValue[i], value);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_RxSignalDiagDebugInvalidValue[i], invalidValue);
...
static void GatewaySwc_UpdateComModeFromBusActivity(void)
{
    /* AI model and CPU telemetry remain scheduled when the vehicle buses are
     * silent. Keep their Ethernet channel available without waking those buses.
     * A separate user also survives ComM's cyclic gateway bus-activity update.
     */
    (void)ComM_RequestComMode(COMM_USER_GATEWAY_ETH,
            (SysMgr_BusActivityCounter > 0u) ?
                    COMM_FULL_COMMUNICATION : COMM_NO_COMMUNICATION);

    if ((SysMgr_BusActivityCounter > 0u) ||
        (Dcm_HasActiveEthernetDiagnosticConnection() != FALSE))
    {
...
static void GatewaySwc_ReportRxMessageTimeoutToDem(uint16 index, uint8 status)
{
    ...
    {
        if (Dem_GetEventFailed(eventId, &eventFailed) == E_OK)
        {
            GatewaySwc_DemMessageTimeoutState[index] =
                    (eventFailed != FALSE) ?
                            (uint8)DEM_EVENT_STATUS_FAILED :
                            (uint8)DEM_EVENT_STATUS_PASSED;
        }
        else if (desiredStatus == DEM_EVENT_STATUS_PASSED)
        {
...
static void GatewaySwc_FlushEthFrame(uint16 *len)
{
    ...
    GatewaySwc_Status.ethLastPayloadLength = *len;
    if (GatewaySwc_QueueEthTx(GATEWAYSWC_ETH_TX_KIND_SUMMARY,
            (SoAd_SoConIdType)GATEWAYSWC_ETH_SOCON_ID,
            NULL_PTR,
            GatewaySwc_EthBuffer,
            *len) == FALSE)
...
void GatewaySwc_ReportDtcTransition(Dem_DTCType dtc, Dem_UdsStatusByteType status)
{
    ...
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugDtcDropped);
        return;
    }
...
static void GatewaySwc_ProcessCore0Async(void)
{
    GatewaySwc_ProcessEthTxResults();
    GatewaySwc_ProcessEthRxQueue();
}

static void GatewaySwc_ProcessEthTxResults(void)
{
    GatewaySwc_EthTxResultType result;

    while (GatewaySwc_PopEthTxResult(&result) != FALSE)
    {
        GatewaySwc_ApplyEthTxResult(&result);
    }
}

static void GatewaySwc_ProcessEthRxQueue(void)
{
    GatewaySwc_EthRxJobType job;

    while (GatewaySwc_PopEthRx(&job) != FALSE)
    {
        GatewaySwc_HandleEthRxMessage(job.soConId,
                (job.hasRemoteAddr != 0u) ? &job.remoteAddr : NULL_PTR,
                job.data,
                job.len);
    }
}
...
static void GatewaySwc_ProcessEthTxQueueCore2(void)
{
    GatewaySwc_EthTxJobType job;
    uint8 openResult;
    uint8 budget = GATEWAYSWC_ETH_TX_DRAIN_BUDGET;
    sint32 result;

    while ((budget > 0u) && (GatewaySwc_PopEthTx(&job) != FALSE))
    {
        openResult = 0u;

        if (job.kind == GATEWAYSWC_ETH_TX_KIND_MCU_STATUS)
        {
            result = GatewaySwc_Core2TransmitMcuStatus(&job);
        }
        else
        {
            result = (sint32)GatewaySwc_Core2TransmitSoAd(&job, &openResult);
        }

        (void)GatewaySwc_QueueEthTxResult(job.kind, openResult, job.len, result);
        budget--;
    }
}
...
static boolean GatewaySwc_GetEthTxCodingPduId(uint8 kind, PduIdType *pduId)
{
    ...
    switch (kind)
    {
        case GATEWAYSWC_ETH_TX_KIND_SUMMARY:
            *pduId = CODINGAPP_TX_PDU_ETH_SUMMARY;
            break;
        case GATEWAYSWC_ETH_TX_KIND_DTC_TRANSITION:
            *pduId = CODINGAPP_TX_PDU_ETH_DTC_TRANSITION;
            break;
        case GATEWAYSWC_ETH_TX_KIND_MCU_STATUS:
            *pduId = CODINGAPP_TX_PDU_ETH_MCU_STATUS;
            break;
        case GATEWAYSWC_ETH_TX_KIND_AI_MODEL:
            *pduId = CODINGAPP_TX_PDU_ETH_AI_MODEL;
            break;
        case GATEWAYSWC_ETH_TX_KIND_CPU_PERF:
            *pduId = CODINGAPP_TX_PDU_ETH_CPU_PERF;
            break;
        case GATEWAYSWC_ETH_TX_KIND_VEHICLESTATE:
            *pduId = CODINGAPP_TX_PDU_ETH_VEHICLESTATE;
            break;
        default:
            return FALSE;
    }
...
static boolean GatewaySwc_IsEthTxKindEnabled(uint8 kind)
{
    PduIdType codingPduId;

    if (GatewaySwc_GetEthTxCodingPduId(kind, &codingPduId) == FALSE)
    {
        return TRUE;
    }

    return CodingApp_IsTxPduEnabled(codingPduId);
}
        ...
        uint16 len)
{
    ...
    if ((data == NULL_PTR) || (len == 0u) || (len > GATEWAYSWC_ETH_MAX_PAYLOAD))
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugEthTxQueueDropped);
        return FALSE;
    }

    if (GatewaySwc_IsEthTxKindEnabled(kind) == FALSE)
    {
        /* Coding-disabled frames are intentionally suppressed, not failed. */
        return TRUE;
    }

    if (GatewaySwc_EnterCrossCoreCritical() == FALSE)
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugEthTxQueueDropped);
        return FALSE;
    }

    nextHead = (uint8)((GatewaySwc_EthTxHead + 1u) % GATEWAYSWC_ETH_TX_QUEUE_SIZE);
    if (nextHead == GatewaySwc_EthTxTail)
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugEthTxQueueDropped);
        GatewaySwc_ExitCrossCoreCritical();
        return FALSE;
    }

    job = &GatewaySwc_EthTxQueue[GatewaySwc_EthTxHead];
    job->kind = kind;
    job->soConId = soConId;
    job->hasRemoteAddr = (remoteAddr != NULL_PTR) ? 1u : 0u;
    if (remoteAddr != NULL_PTR)
    {
        job->remoteAddr = *remoteAddr;
    }
    else
    {
        job->remoteAddr.addr = 0u;
        job->remoteAddr.port = 0u;
    }
    job->len = len;
    for (i = 0u; i < len; i++)
    {
...
static uint8 GatewaySwc_PopEthTx(GatewaySwc_EthTxJobType *job)
{
    ...
    if (GatewaySwc_EthTxTail != GatewaySwc_EthTxHead)
    {
        *job = GatewaySwc_EthTxQueue[GatewaySwc_EthTxTail];
        GatewaySwc_EthTxTail =
            (uint8)((GatewaySwc_EthTxTail + 1u) % GATEWAYSWC_ETH_TX_QUEUE_SIZE);
        popped = TRUE;
    }
...
static uint8 GatewaySwc_QueueEthTxResult(uint8 kind,
        ...
        sint32 result)
{
    GatewaySwc_EthTxResultType *entry;
    uint8 nextHead;

    if (GatewaySwc_EnterCrossCoreCritical() == FALSE)
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugEthTxResultQueueDropped);
        return FALSE;
    }

    nextHead = (uint8)((GatewaySwc_EthTxResultHead + 1u) % GATEWAYSWC_ETH_TX_RESULT_QUEUE_SIZE);
    if (nextHead == GatewaySwc_EthTxResultTail)
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugEthTxResultQueueDropped);
        GatewaySwc_ExitCrossCoreCritical();
        return FALSE;
    ...
    entry = &GatewaySwc_EthTxResultQueue[GatewaySwc_EthTxResultHead];
    entry->kind = kind;
    entry->openResult = openResult;
    entry->len = len;
    entry->result = result;

    __dsync();
    GatewaySwc_EthTxResultHead = nextHead;
    GatewaySwc_ExitCrossCoreCritical();
...
static uint8 GatewaySwc_PopEthTxResult(GatewaySwc_EthTxResultType *result)
{
    ...
    if (GatewaySwc_EthTxResultTail != GatewaySwc_EthTxResultHead)
    {
        *result = GatewaySwc_EthTxResultQueue[GatewaySwc_EthTxResultTail];
        GatewaySwc_EthTxResultTail =
            (uint8)((GatewaySwc_EthTxResultTail + 1u) % GATEWAYSWC_ETH_TX_RESULT_QUEUE_SIZE);
        popped = TRUE;
    }
        ...
        uint16 len)
{
    ...
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugEthRxQueueDropped);
        return FALSE;
    }

    copyLen = len;
    if (copyLen > GATEWAYSWC_ETH_RX_QUEUE_MAX_PAYLOAD)
    {
        copyLen = GATEWAYSWC_ETH_RX_QUEUE_MAX_PAYLOAD;
        GATEWAYSWC_DEBUG_INC(GatewaySwc_EthRxCaptureTruncatedCounter);
    }
    ...
    nextHead = (uint8)((GatewaySwc_EthRxHead + 1u) % GATEWAYSWC_ETH_RX_QUEUE_SIZE);
    if (nextHead == GatewaySwc_EthRxTail)
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugEthRxQueueDropped);
        GatewaySwc_ExitCrossCoreCritical();
        return FALSE;
    ...
    job = &GatewaySwc_EthRxQueue[GatewaySwc_EthRxHead];
    job->soConId = soConId;
    job->hasRemoteAddr = (remoteAddr != NULL_PTR) ? 1u : 0u;
    if (remoteAddr != NULL_PTR)
    {
...
static uint8 GatewaySwc_PopEthRx(GatewaySwc_EthRxJobType *job)
{
    ...
    if (GatewaySwc_EthRxTail != GatewaySwc_EthRxHead)
    {
        *job = GatewaySwc_EthRxQueue[GatewaySwc_EthRxTail];
        GatewaySwc_EthRxTail =
            (uint8)((GatewaySwc_EthRxTail + 1u) % GATEWAYSWC_ETH_RX_QUEUE_SIZE);
        popped = TRUE;
    }
...
static void GatewaySwc_ApplyEthTxResult(const GatewaySwc_EthTxResultType *result)
{
    ...
    GatewaySwc_Status.ethLastPayloadLength = result->len;

    if (result->kind == GATEWAYSWC_ETH_TX_KIND_MCU_STATUS)
    {
        GatewaySwc_McuStatusTxPending = 0u;
        if (result->result == (sint32)result->len)
        {
    ...
    GatewaySwc_Status.ethLastOpenResult = result->openResult;
    GatewaySwc_Status.ethLastSoAdResult = (uint8)result->result;

    if (result->kind == GATEWAYSWC_ETH_TX_KIND_DTC_TRANSITION)
    {
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastOpenResult, result->openResult);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastTxResult, (uint8)result->result);
    }

    if ((SoAd_ReturnType)result->result == SOAD_OK)
    {
        GatewaySwc_Status.ethLastTransmitOk = 1u;
        GatewaySwc_Status.ethFramesSent++;
    }
        ...
        uint8 *openResult)
{
    ...
    if ((job == NULL_PTR) || (openResult == NULL_PTR))
    {
        return SOAD_NOT_OK;
    }
    ...
    *openResult = SoAd_OpenSoCon(job->soConId);
    remoteAddr = (job->hasRemoteAddr != 0u) ? &job->remoteAddr : NULL_PTR;

    return GatewaySwc_RequestSoAdIfTransmit(job->soConId,
            remoteAddr,
            job->data,
            job->len);
}
...
static sint32 GatewaySwc_Core2TransmitMcuStatus(const GatewaySwc_EthTxJobType *job)
{
    ...
    return GatewaySwc_RequestTcpIpSendTo(GatewaySwc_McuStatusSocket,
            &remoteAddr,
            job->data,
            job->len);
#else
...
static uint8 GatewaySwc_EnterCrossCoreCritical(void)
{
    ...
    GatewaySwc_CrossCoreIrqState[coreIndex] = IfxCpu_disableInterrupts();

    if (IfxCpu_setSpinLock(&GatewaySwc_CrossCoreLock, GATEWAYSWC_CROSS_CORE_LOCK_TIMEOUT) != FALSE)
    {
        GatewaySwc_CrossCoreLockOwned[coreIndex] = TRUE;
        __dsync();
    }
    else
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugCrossCoreLockTimeout);
        IfxCpu_restoreInterrupts(GatewaySwc_CrossCoreIrqState[coreIndex]);
        return FALSE;
    }
...
static void GatewaySwc_ExitCrossCoreCritical(void)
{
    ...
    if (GatewaySwc_CrossCoreLockOwned[coreIndex] != FALSE)
    {
        GatewaySwc_CrossCoreLockOwned[coreIndex] = FALSE;
        IfxCpu_resetSpinLock(&GatewaySwc_CrossCoreLock);
    }
...
static uint8 GatewaySwc_EnterDtcQueueCritical(void)
{
    ...
    {
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugDtcQueueLockTimeout);
...
static void GatewaySwc_UpdateDtcDebugQueueState(void)
{
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcQueueHead, GatewaySwc_DtcTransitionHead);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcQueueTail, GatewaySwc_DtcTransitionTail);

    if (GatewaySwc_DtcTransitionHead >= GatewaySwc_DtcTransitionTail)
    {
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcQueueCount,
...
static void GatewaySwc_FlushDtcTransitionQueue(void)
{
    ...
    while (GatewaySwc_PopDtcTransition(&transition) != FALSE)
    {
        ...
        GATEWAYSWC_DEBUG_INC(GatewaySwc_DebugDtcFlushed);
    }
}
...
static void GatewaySwc_PublishDtcTransition(Dem_DTCType dtc, Dem_UdsStatusByteType status)
{
    ...
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastDtc, dtc & 0x00FFFFFFu);
    GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastStatus, status);
    ...
    if (GatewaySwc_QueueEthTx(GATEWAYSWC_ETH_TX_KIND_DTC_TRANSITION,
            (SoAd_SoConIdType)GATEWAYSWC_ETH_SOCON_ID,
            NULL_PTR,
            payload,
            GATEWAYSWC_DTC_TRANSITION_LEN) == FALSE)
    {
        GatewaySwc_Status.ethLastOpenResult = 0u;
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastOpenResult, 0u);
        GATEWAYSWC_DEBUG_ASSIGN(GatewaySwc_DebugDtcLastTxResult, SOAD_NOT_OK);
